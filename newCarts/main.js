...
let limit = 2;
function fetchCarts() {
    fetch(`${URL}/carts?_limit=${limit}&per_page=${perPageCart}&_page=${pageCart}`)
        .then((res) => {
            if (!res.ok) {
        ...
        .then((data) => {
            const { data: dataCartRes, pagination } = data;
            carts = dataCartRes;
            renderCarts(carts);
            renderPaginationCart(Math.ceil(pagination._totalRows / perPageCart));
        })
        .catch((error) => {
            alert(error);
        })
...
function renderPaginationCart(totalPagination) {
    inner($(".pagination"), "");
    for (let index = 1; index <= totalPagination; index++) {
        $(".pagination").innerHTML += `
        <li onclick="changePagination(${index})" class="${index === pageCart ? "hidden" : ""}">${index}</li>`;
    }
}
function changePagination(changeNumber) {
    pageCart = changeNumber;
    loadingToggle();
    fetchCarts();
}
function plus(id) {
